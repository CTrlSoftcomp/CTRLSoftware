...
SELECT ISNULL(@NoID, 0) + ROW_NUMBER() OVER(ORDER BY NoID) [NoID]
           ...
           ,0 [IsDefault]
           ,[IsActive]
           ,CASE WHEN ISNULL(HargaBeliPcs, 0)*IsiCtn=0 THEN 0 ELSE ROUND(((ISNULL(HargaJualA1, 0)-(ISNULL(HargaBeliPcs, 0)*IsiCtn))/(ISNULL(HargaBeliPcs, 0)*IsiCtn))*100, 2) END AS[ProsenUpA]
           ,[HargaJualA1]
           ,-100 [ProsenUpB]
           ,[HargaJualB1]
FROM MBarang
