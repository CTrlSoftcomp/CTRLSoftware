                                ...
                                Try
                                    ...
                                    If Not DxErrorProvider1.HasErrors Then
                                        If pStatus = pStatusForm.Baru Then
                                            ...
                                            com.CommandText = "INSERT INTO [dbo].[MReturJualD] ([NoID],[IDHeader],[IDBarangD],[IDBarang],[IDSatuan],[Konversi],[Qty]" & vbCrLf &
                                        ...
                                        Else
                                            com.CommandText = "UPDATE [dbo].[MReturJualD] SET [IDBarangD]=@IDBarangD,[IDBarang]=@IDBarang,[IDSatuan]=@IDSatuan,[Konversi]=@Konversi,[Qty]=@Qty" & vbCrLf &
                                        ...
                                        com.CommandText = "UPDATE MReturJual SET Subtotal=ISNULL(MReturJualD.JumlahBruto, 0), TotalBruto=ISNULL(MReturJualD.JumlahBruto, 0), " & vbCrLf &
                                                          "DPP=ROUND(CASE WHEN MReturJual.IDTypePajak=0 THEN 0 WHEN MReturJual.IDTypePajak=2 THEN ISNULL(MReturJualD.JumlahBruto, 0)/1.0 ELSE ISNULL(MReturJualD.JumlahBruto, 0)/1.1 END, 0) " & vbCrLf &
                                                          "FROM MReturJual " & vbCrLf &
                                                          "INNER JOIN (SELECT IDHeader, SUM(JumlahBruto) AS JumlahBruto FROM MReturJualD GROUP BY IDHeader) AS MReturJualD ON MReturJualD.IDHeader=MReturJual.NoID " & vbCrLf &
                                                          "WHERE MReturJual.NoID=" & IDHeader
                                        com.ExecuteNonQuery()

                                        com.CommandText = "UPDATE MReturJual SET " & vbCrLf &
                                                          "PPN=ROUND((CASE WHEN MReturJual.IDTypePajak=0 THEN 0 WHEN MReturJual.IDTypePajak=1 THEN ISNULL(MReturJualD.JumlahBruto, 0)-ISNULL(MReturJualD.DPP, 0) ELSE 0.1*ISNULL(MReturJualD.JumlahBruto, 0) END), 0) " & vbCrLf &
                                                          "FROM MReturJual " & vbCrLf &
                                                          "INNER JOIN (SELECT IDHeader, SUM(JumlahBruto) AS JumlahBruto, SUM(DPP) AS DPP, SUM(PPN) AS PPN FROM MReturJualD GROUP BY IDHeader) AS MReturJualD ON MReturJualD.IDHeader=MReturJual.NoID " & vbCrLf &
                                                          "WHERE MReturJualD.IDHeader=" & IDHeader
                                        com.ExecuteNonQuery()

                                        com.CommandText = "UPDATE MReturJualD SET " & vbCrLf &
                                                          "PPN=ROUND((CASE WHEN MReturJual.IDTypePajak=0 THEN 0 WHEN MReturJual.IDTypePajak=1 THEN ISNULL(MReturJualD.JumlahBruto, 0)-ISNULL(MReturJualD.DPP, 0) ELSE 0.1*ISNULL(MReturJualD.JumlahBruto, 0) END), 0) " & vbCrLf &
                                                          "FROM MReturJual " & vbCrLf &
                                                          "INNER JOIN MReturJualD ON MReturJualD.IDHeader=MReturJual.NoID " & vbCrLf &
                                                          "WHERE MReturJualD.IDHeader=" & IDHeader
                                        com.ExecuteNonQuery()
                                        ...
                                        com.CommandText = "UPDATE MReturJualD SET " & vbCrLf &
                                                          "PPN=ISNULL(MReturJual.PPN, 0)-(ISNULL(Detil.PPN, 0)-ISNULL(MReturJualD.PPN, 0)), " & vbCrLf &
                                                          "DPP=ISNULL(MReturJual.DPP, 0)-(ISNULL(Detil.DPP, 0)-ISNULL(MReturJualD.DPP, 0)) " & vbCrLf &
                                                          "FROM MReturJual " & vbCrLf &
                                                          "INNER JOIN MReturJualD ON MReturJualD.IDHeader=MReturJual.NoID" & vbCrLf &
                                                          "INNER JOIN (SELECT IDHeader, SUM(DPP) AS DPP, SUM(PPN) AS PPN, MAX(NoID) AS NoID FROM MReturJualD GROUP BY IDHeader) AS Detil ON Detil.IDHeader=MReturJual.NoID AND Detil.NoID=MReturJualD.NoID" & vbCrLf &
                                                          "WHERE MReturJualD.IDHeader=" & IDHeader
                                        com.ExecuteNonQuery()

                                        com.CommandText = "UPDATE MReturJualD SET " & vbCrLf &
                                                          "Jumlah=CASE WHEN MReturJual.IDTypePajak=0 THEN MReturJualD.JumlahBruto ELSE MReturJualD.DPP+MReturJualD.PPN END " & vbCrLf &
                                                          "FROM MReturJual " & vbCrLf &
                                                          "INNER JOIN MReturJualD ON MReturJualD.IDHeader=MReturJual.NoID " & vbCrLf &
                                                          "WHERE MReturJualD.IDHeader=" & IDHeader
                                        com.ExecuteNonQuery()

                                        com.CommandText = "UPDATE MReturJual SET Subtotal=ISNULL(MReturJualD.JumlahBruto, 0), TotalBruto=ISNULL(MReturJualD.JumlahBruto, 0), Total=ISNULL(MReturJualD.Jumlah, 0), " & vbCrLf &
                                                          "Bayar=ISNULL(MReturJualDBayar.TotalBayar, 0), Sisa=ISNULL(MReturJualD.Jumlah, 0)+ISNULL(MReturJualDBayar.ChargeRp, 0)-ISNULL(MReturJualDBayar.TotalBayar, 0)" & vbCrLf &
                                                          "FROM MReturJual " & vbCrLf &
                                                          "LEFT JOIN (SELECT IDHeader, SUM(JumlahBruto) AS JumlahBruto, SUM(Jumlah) AS Jumlah FROM MReturJualD GROUP BY IDHeader) AS MReturJualD ON MReturJualD.IDHeader=MReturJual.NoID " & vbCrLf &
                                                          "LEFT JOIN (SELECT IDHeader, SUM(Total) AS TotalBayar, SUM(ChargeRp) AS ChargeRp FROM MReturJualDBayar GROUP BY IDHeader) AS MReturJualDBayar ON MReturJualDBayar.IDHeader=MReturJual.NoID " & vbCrLf &
                                                          "WHERE MReturJual.NoID=" & IDHeader
                                        com.ExecuteNonQuery()
