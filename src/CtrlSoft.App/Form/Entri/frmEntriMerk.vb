                            ...
                            Using ds As New DataSet
                                Try
                                    ...
                                    If pStatus = pStatusForm.Baru Then
                                        com.CommandText = "SELECT MAX(NoID) FROM MMerk"
                                        NoID = NullToLong(com.ExecuteScalar()) + 1

                                        com.CommandText = "INSERT INTO MMerk (NoID, Kode, Nama, IsActive) VALUES (@NoID, @Kode, @Nama, @IsActive)"
                                    Else
                                        com.CommandText = "UPDATE MMerk SET Kode=@Kode, Nama=@Nama, IsActive=@IsActive WHERE NoID=@NoID"
                                    End If
                    ...
                    Using oDA As New SqlDataAdapter
                        Using ds As New DataSet
                            Try
                                ...
                                com.CommandText = "SELECT * FROM MMerk WHERE NoID=" & NoID
                                oDA.Fill(ds, "MMerk")

                                If ds.Tables("MMerk").Rows.Count >= 1 Then
                                    pStatus = pStatusForm.Edit
                                    Me.NoID = NullToLong(ds.Tables("MMerk").Rows(0).Item("NoID"))
                                    txtUserID.Text = NullToStr(ds.Tables("MMerk").Rows(0).Item("Kode"))
                                    txtNama.Text = NullToStr(ds.Tables("MMerk").Rows(0).Item("Nama"))
                                    ckAktif.Checked = NullToBool(ds.Tables("MMerk").Rows(0).Item("IsActive"))
                                Else
